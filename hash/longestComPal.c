...
int stringhash1(char* x, int len)
{
    ...
    for(int i=0;i<len;i++)
    {
        result = (result%p+(((int)x[i]-(int)'a')*(int)pow(D,len-i-1))%p)%p;
    }
    return result;
}
...
int stringhash2(char* x, int len)
{
    ...
    for(int i=0;i<len;i++)
    {
        result = (result%p+(((int)x[i]-(int)'a')*(int)pow(D,i))%p)%p;
    }
    return result;
}
...
int searchHashEven(char* string, int len, node* arr[])
{
    ...
    int p = 34369934;
    int i,j,k;
    for(i=0,j=strlen(string)-len/2-1,k=0;i<len/2;i++,j--,k++) 
    {
        firstStrPrefix[i] = string[i];
        firstStrSuffix[k] = rev[j];
    }
    firstStrPrefix[i] = '\0';
    firstStrSuffix[k] = '\0';
    printf("%s %s\n",firstStrPrefix,firstStrSuffix);
    int hash1 = stringhash1(firstStrPrefix,len/2);
    int hash2 = stringhash2(firstStrSuffix,len/2);
    printf("hash1 = %d hash2 = %d\n",hash1,hash2);
    if(hash1==hash2) if(HashTableSearch(hash1,arr)) return 1;
    for(int i=1,j=strlen(string)-len/2-2; j>=len/2-1 ;i++,j--)
    {
        ...
        hash1 = (hash1 * (D%p))%p;
        hash1 = (hash1-(((int)string[i-1])*(int)pow(D,len/2))%p)%p;
        if(hash1<0) hash1 += p;
        hash1 = (hash1 + ((int)string[i+len/2-1])%p)%p;
        hash2 = (hash2 * (D%p))%p;
        hash2 = (hash2-(((int)rev[j+1])*(int)pow(D,len/2))%p)%p;
        if(hash2<0) hash2 += p;
        hash2 = (hash2 + ((int)rev[j-len/2+1])%p)%p;
        if(hash1==hash2) if(HashTableSearch(hash1,arr)) return 1;
        printf("hash1 = %d hash2 = %d\n",hash1,hash2);
    }
...
int findHashEven(char* string, int len, node* arr[])
{
    printf("len = %d\n",len);
    if(len/2==0) return 1;
    char* rev = (char*)malloc((strlen(string))*sizeof(char));
...
int searchHashOdd(char* string, int len, node* arr[])
{
    ...
    int p = 34369934;
    int i,j,k;
    for(i=0,j=strlen(string)-len/2-2,k=0;i<len/2;i++,j--,k++) 
    {
        firstStrPrefix[i] = string[i];
        firstStrSuffix[k] = rev[j];
    }
...
int findHashOdd(char* string, int len, node* arr[])
{
    printf("len = %d\n",len);
    if(len/2==0) return 1;
    char* rev = (char*)malloc((strlen(string))*sizeof(char));
...
int longestCommonPal(char* s1, char* s2)
{
    node* arr[101513];
    int l1 = strlen(s1);
    int l2 = strlen(s2);
    ...
    while(beg<=end)
    {
        ...
        if(mid%2 == 0)
        {
            findHashEven(s1,mid,arr);
            if(searchHashEven(s2,mid,arr))
            {
                ans = mid;
                beg = mid+1;
            }
            ...
            {
                findHashOdd(s1,mid+1,arr);
                if(searchHashOdd(s2,mid+1,arr))
                {
                    ans = mid+1;
                    beg = mid+2;
                }
            ...
            findHashOdd(s1,mid,arr);
            if(searchHashOdd(s2,mid,arr))
            {
                ans = mid;
                beg = mid+1;
            }
            ...
            {
                findHashEven(s1,mid+1,arr);
                if(searchHashEven(s2,mid+1,arr))
                {
                    ans = mid+1;
                    beg = mid+2;
                }
...
int main()
{
    ...
    scanf("%s",s2);
    int len = longestCommonPal(s1,s2);
